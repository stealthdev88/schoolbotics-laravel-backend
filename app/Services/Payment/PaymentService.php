...
class PaymentService {
    ...
    public static function create(string $paymentGateway, $schoolID = null) {
        ...
        Log::channel('custom')->error('PaymentService payment:' . json_encode($payment));
        Log::channel('custom')->error('PaymentService secret_key111:' . $payment->secret_key);
        Log::channel('custom')->error('PaymentService currency_code222:' . $payment->currency_code);
        return match ($paymentGateway) {
            'stripe' => new StripePayment($payment->secret_key, $payment->currency_code),
            'paystack' => new PaystackPayment($payment->secret_key, $payment->currency_code),
