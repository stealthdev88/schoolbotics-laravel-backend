...
//            $student = $this->student->findById($request->child_id, ['id', 'user_id', 'class_section_id', 'school_id'], ['class_section']);
    ...
    public function payCompulsoryFees(Request $request) {
        Log::channel('custom')->error("parent pay CompulusoryFees request:" . json_encode($request));
        $validator = Validator::make($request->all(), [
            'child_id'          => 'required',
            'fees_id'           => 'required',
        ...
        try {
            DB::beginTransaction();
            Log::channel('custom')->error("parent pay CompulusoryFees payment_method:" . $request->payment_method);
            ...
            Log::channel('custom')->error('guardian email:' . Auth::user()->email);
            $studentData = Auth::user()->guardianRelationChild()->where('id', $request->child_id)->whereHas('user', function ($q) {
                $q->whereNull('deleted_at');
            })->first();
...
//                    }
            ...
            Log::channel('custom')->error("parent pay CompulusoryFees paymentTransactionData:" . json_encode($paymentTransactionData));
            $paymentGateway = PaymentService::create($request->payment_method, $schoolId);
            Log::channel('custom')->error("parent pay CompulusoryFees paymentGateway:" . json_encode($paymentGateway));
            $paymentIntent = $paymentGateway->createPaymentIntent(round($finalAmount, 2), [
            ...
            Log::channel('custom')->error("parent pay CompulusoryFees paymentIntent:" . json_encode($paymentIntent));

            $this->paymentTransaction->update($paymentTransactionData->id, ['order_id' => $paymentIntent->reference, 'school_id' => $schoolId]);

            Log::channel('custom')->error("parent pay paymentTransaction updated");

            $paymentTransactionData = $this->paymentTransaction->findById($paymentTransactionData->id);

            Log::channel('custom')->error("parent pay CompulusoryFees paymentTransactionData_id:" . json_encode($paymentTransactionData->id));
            ...
            //     'payment_transaction_id' => $paymentTransactionData->id,
            // );
            $paymentGatewayDetails = json_decode(json_encode($paymentIntent), true);

            Log::channel('custom')->error("parent pay CompulusoryFees paymentIntent array data:" . json_encode($paymentGatewayDetails));

            $paymentGatewayDetails['payment_transaction_id'] = $paymentTransactionData->id;
            $paymentGatewayDetails['email'] = Auth::user()->email;
            $paymentGatewayDetails['currency'] = $paymentConfigurations->currency_code;

            Log::channel('custom')->error("parent pay paymentTransaction commited");
            
            DB::commit();

            Log::channel('custom')->error("parent pay CompulusoryFees payment_intent:" . json_encode($paymentGatewayDetails));
            
            ResponseService::successResponse("", ["payment_intent" => $paymentGatewayDetails, "payment_transaction" => $paymentTransactionData]);
        } catch (Throwable $e) {
            DB::rollBack();
            Log::channel('custom')->error("parent pay CompulusoryFees error:" . json_encode($e));
            ResponseService::logErrorResponse($e);
            ResponseService::errorResponse();
        }
    }
