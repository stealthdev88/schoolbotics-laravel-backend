...
class AddonController extends Controller {
    ...
    public function plan() {
        ...
        return view('addons.plan', compact('addons', 'settings','subscription','features'));
    }
    ...
    public function status($id) {
        ...
        try {
            ...
            $addon = $this->addon->findById($id);
            Log::channel('custom')->error('addon:' . json_encode($addon));
            DB::commit();
            
            // if ($addon['status'] == 1) {
            //     $settings = app(CachingService::class)->getSystemSettings();
            //     Log::channel('custom')->error('settings:' . json_encode($settings));
            //     Log::channel('custom')->error('addon_name:' . $addon->name);
            //     $placeholder = $this->replacePlaceholdersForSMSForSuperAdmin(Auth::user(), $settings, $addon->name);
            //     Log::channel('custom')->error('placeholder:' . json_encode($placeholder));
            //     $this->sendSMS("Super Admin", "addon_purchase_activate", $placeholder);    
            // }
    ...
    public function subscribe($id, $type) {
        ...
        try {
            ...
            $settings = app(CachingService::class)->getSystemSettings();
            $text = $addon['name'] . " to Add to your current " . $subscription['name'];
            $placeholder = $this->replacePlaceholdersForSMS($addonSubscription->school_id, Auth::user(), $settings, $text);
            $this->sendSMSForAddOn("Super Admin", "addon_purchase_activate", $placeholder, $addonSubscription->school_id);
    ...
    public function payment_success_paystack($id)
    {
        $settings = app(CachingService::class)->getSystemSettings();
        ...
        Log::channel('custom')->error('payment_success_paystack trxref:' . $trxref);
        Log::channel('custom')->error('payment_success_paystack reference:' . $reference);
        ...
        // $this->subscriptionBill->findById($id);
        Log::channel('custom')->error('payment_success_paystack paymentConfiguration:' . json_encode($paymentConfiguration));
        ...
        Log::channel('custom')->error('payment_success_paystack paymentConfiguration step1');
        $payment_data = [
            'user_id'         => Auth::user()->id,
        ...
        $paymentTransaction = $this->paymentTransaction->create($payment_data);
        Log::channel('custom')->error('payment_success_paystack paymentConfiguration step2');
        $addonSubscription = $this->addonSubscription->update($id, ['payment_transaction_id' => $paymentTransaction->id]);
        Log::channel('custom')->error('payment_success_paystack paymentConfiguration step3');
        // $stripe = new StripeClient($stripe_secret_key);
        // $stripeData = $stripe->customers->create(
        //     [
        //         'metadata' => [
        ...
        // Log::channel('custom')->error('settings:' . json_encode($settings));
        // $placeholder = $this->replacePlaceholdersForSMS($addonSubscription->school_id, Auth::user(), $settings);
        // $this->sendSMS("Super Admin", "addon_purchase_activate", $placeholder, $addonSubscription->school_id);

        $this->cache->removeSchoolCache(config('constants.CACHE.SCHOOL.FEATURES'), $addonSubscription->school_id);
        Log::channel('custom')->error('payment_success_paystack paymentConfiguration step4');
        return redirect()->route('addons.plan')->with('success', trans('the_payment_has_been_completed_successfully'));
    }
    ...
    private function replacePlaceholdersForSMS($school_id, $user, $settings, $text)
    {
        $school_admin_name = DB::select("select concat(first_name, ' ', last_name) full_name from users where id = " . $user->id)[0]->full_name;
        $school_name = DB::select("select name from schools where id = " . $school_id)[0]->name;
    ...
    private function replacePlaceholdersForSMSForSuperAdmin($user, $settings, $text)
    {
        // Define the placeholders and their replacements
        $placeholders = [
            '{email}' => $user->email,
