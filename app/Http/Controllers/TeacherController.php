...
class TeacherController extends Controller {
    ...
    public function __construct(StaffInterface $staff, UserInterface $user, SubscriptionInterface $subscription, CachingService $cache, SubscriptionService $subscriptionService, PayrollSettingInterface $payrollSetting, StaffSalaryInterface $staffSalary, SchoolInterface $schoolsRepository1) {
        $this->user = $user;
        $this->staff = $staff;
    ...
    public function store(Request $request) {
        ...
        try {
            ...
            $this->sendSMSForTeacher(Auth::user()->school_id, "teacher_created", $placeholder, $user);    

            Log::channel('custom')->error('user:' . json_encode($user));
            // // other staff
            // $staff = DB::select("select * from staffs sf left join users u on sf.user_id = u.id where u.school_id = " . $user->school_id . " and u.status = 1");

            // $text = $user->email . ' teacher created \n';
            // $placeholder = $this->replacePlaceholdersForSMS(Auth::user(), $settings, $text);
            // Log::channel('custom')->error('placeholder:' . json_encode($placeholder));
            // $this->sendSMSForTeacherToStaff(Auth::user()->school_id, "teacher_created", $placeholder, $staff);    
    ...
    private function replacePlaceholdersForSMS($user, $settings, $text = '', $text1 = '')
    {
        ...
        Log::channel('custom')->error('school_id:' . $school_id);
        ...
        Log::channel('custom')->error('school_admin_name:' . json_encode($school_admin_name));
    ...
    public function changeStatus($id) {
        ...
        try {
            ...
            Log::channel('custom')->error('teacher info :' . json_encode($teacher));
            $placeholder = $this->replacePlaceholdersForSMS(Auth::user(), $settings, ($teacher->status == 0 ? "teacher account deactived" : "teacher account actived"));
            Log::channel('custom')->error('placeholder:' . json_encode($placeholder));
            $this->sendSMSForTeacher(Auth::user()->school_id, "teacher_created", $placeholder, $teacher);    
