...
class SubscriptionController extends Controller
{
    ...
    public function __construct(PackageInterface $package, FeatureInterface $feature, SubscriptionInterface $subscription, AddonSubscriptionInterface $addonSubscription, UserInterface $user, SchoolSettingInterface $schoolSettings, StaffInterface $staff, SubscriptionBillInterface $subscriptionBill, PaymentTransactionInterface $paymentTransaction, SchoolInterface $school, SchoolSettingInterface $schoolSetting, CachingService $cachingService, SubscriptionFeatureInterface $subscriptionFeature, PaymentConfigurationInterface $paymentConfiguration, SubscriptionService $subscriptionService, SchoolInterface $schoolsRepository)
    {
        ...
        $this->schoolsRepository1 = $schoolsRepository;
    }
     ...
     * @throws ApiErrorException
     */
    public function payment_success_paystack( $subscriptionBill_id = null, $package_id = null, $type = null, $subscription_id = null, $isCurrentPlan = null)
    {
        // check_out_session_id => Stripe check out session
        // subscriptionBill_id => If already bill generated Like postpaid plan run cron job and generate bill
        ...
        $paymentConfiguration = PaymentConfiguration::where('school_id', null)->where('status', 1)->where('payment_method', 'PayStack')->first();

        $paystack_secret_key = $paymentConfiguration->secret_key ?? null;
        $currency = $paymentConfiguration->currency_code;

        $this->paystack = new Paystack($paystack_secret_key);
        $session = $this->paystack->transaction->verify([
            'reference' => $reference
        ]);

        Log::channel('custom')->error('retrievePaymentIntent subscription callback session:' . json_encode($session));

        $status = "pending";
        if ($session->message == "Verification successful") {
            $status = "succeed";
        }
        Log::channel('custom')->error('retrievePaymentIntent subscription callback step1:');
        // Testing

        $subscription = $this->subscriptionService->active_subscription(Auth::user()->school_id);
        if ($subscription) {
            // Postpaid plan generate bill
            Log::channel('custom')->error('retrievePaymentIntent subscription callback step1-1:');
            if ($subscription->package_type == 1) {
                // Create current subscription plan bill
                $subscriptionBillData = $this->subscriptionService->createSubscriptionBill($subscription, null);
            }

            Log::channel('custom')->error('retrievePaymentIntent subscription callback step2:');
            // Update current plan end date & delete features
            $current_subscription_expiry = $this->subscription->update($subscription->id, ['end_date' => Carbon::now()->format('Y-m-d')]);
            $this->subscriptionFeature->builder()->where('subscription_id', $subscription->id)->delete();
            Log::channel('custom')->error('retrievePaymentIntent subscription callback step3:');
            // Update upcoming plan start & end date if found
            $upcoming_package = $this->subscription->builder()->with('package')->whereDate('start_date', '>', $subscription->end_date)->delete();

            // Delete addons
            $addons = $this->addonSubscription->builder()->where('subscription_id', $subscription->id)->get();
            Log::channel('custom')->error('retrievePaymentIntent subscription callback step4:');
            $soft_delete_addon = array();
            foreach ($addons as $key => $addon) {
                $this->addonSubscription->update($addon->id, ['end_date' => $current_subscription_expiry->end_date]);
                $soft_delete_addon[] = $addon->id;
            }
            Log::channel('custom')->error('retrievePaymentIntent subscription callback step5:');
            $this->addonSubscription->builder()->whereIn('id', $soft_delete_addon)->delete();
            Log::channel('custom')->error('retrievePaymentIntent subscription callback step6:');
        }

        Log::channel('custom')->error('retrievePaymentIntent subscription callback step7:');
        // End testing
        ...
        Log::channel('custom')->error('retrievePaymentIntent subscription callback step8:');

        $payment_data = [
            'user_id'         => Auth::user()->id,
            'amount'          => ($session->data->amount / 100),
            'payment_gateway' => 'PayStack',
            'order_id'        => $session->data->reference,
        ...
        Log::channel('custom')->error('retrievePaymentIntent subscription callback step9 ');
        $this->cache->removeSchoolCache(config('constants.CACHE.SCHOOL.FEATURES'), $subscriptionBill->school_id);

        return redirect()->route('subscriptions.history')->with('success', trans('the_payment_has_been_completed_successfully'));
    }

    public function paystack_payment_callback($reference, $subscriptionBill_id = null, $package_id = null, $type = null, $subscription_id = null, $isCurrentPlan = null)
    {
        try {
            $settings = app(CachingService::class)->getSystemSettings();
            $currency = $settings['currency_code'];
            ...
            // Assuming similar logic for handling subscriptions and billing as in the Stripe example
            $subscription = $this->subscriptionService->active_subscription(Auth::user()->school_id);
            if ($subscription) {
                // Similar logic for handling subscription updates and billing
                // ...
            }

            $id = '';
            if ($type != -1) {
                // Similar logic for creating or updating subscriptions based on type
                // ...
            } else {
                if ($subscriptionBill_id != -1) {
                    $id = $subscriptionBill_id;
            ...
            $payment_data = [
                ...
                'payment_gateway' => 'Paystack',
                'order_id'        => $reference,
                'payment_id'      => $verification->data->id,
                'payment_status'  => $status,
            ];
    ...
    public function bill_payment_store(Request $request, $id)
    {
        ...
        Log::channel('custom')->error('subscriptionBill Info:' . json_encode($subscriptionBill));
        ...
        try {
            ...
            $settings = app(CachingService::class)->getSystemSettings();
            $text = DB::select("select * from subscriptions where id = ".  $subscriptionBill->subscription_id)[0]->name;
            $placeholder = $this->replacePlaceholdersForSMS($request, Auth::user(), $settings, $text);
            Log::channel('custom')->error('placeholder:' . json_encode($placeholder));
            $this->sendSMSForSubscriptionActivated("Super Admin", "subscription_activated", $placeholder, $request->school_id);
    ...
    private function replacePlaceholdersForSMS($request, $user, $settings, $text)
    {
        $school_admin_name = User::role("School Admin")->whereHas('school', function ($q) use ($request) {
            $q->whereNull('deleted_at')->where('status', 1)->where('id', $request->school_id);
        })->get()->first();

        $school_name = DB::select("select * from schools where id = " . $school_id)[0]->name;
        // Define the placeholders and their replacements
        $placeholders = [
            '{school_admin_name}' => $school_admin_name->full_name,
            '{email}' => $user->email,
