...
class OnlineExamController extends Controller {
    ...
    public function __construct(ClassSectionInterface $classSection, SubjectTeacherInterface $subjectTeacher, OnlineExamInterface $onlineExam, OnlineExamQuestionChoiceInterface $onlineExamQuestionChoice, OnlineExamQuestionInterface $onlineExamQuestion, OnlineExamQuestionOptionInterface $onlineExamQuestionOption, OnlineExamStudentAnswerInterface $onlineExamStudentAnswer, CachingService $cachingService, StudentInterface $student, StudentOnlineExamStatusInterface $studentOnlineExamStatus, ClassSubjectInterface $classSubjects, SessionYearInterface $sessionYear, SchoolInterface $schoolsRepository1) {
    ...
    public function store(Request $request) {
        ...
        try {
            ...
            $sql = "select * from (select s.*, cs.section_id, cs.class_id, concat(c.name, '(', Nvl(st.name, ''), ')', nvl(sh.name, ''), '-', nvl(sh.name, '')) class_name, concat(u.first_name, ' ',u.last_name) full_name, concat(g.first_name, ' ', g.last_name) guardian_name, u.mobile mobile_s, g.mobile mobile_g " . 
                "from students s " . 
                ...
                ") s where s.section_id = " . $request->class_section_id;
            ...
            $sql = "select ct.teacher_id, u.mobile, u.first_name, u.last_name from subject_teachers ct left join users u on ct.teacher_id = u.id where ct.class_section_id = " . $request->class_section_id . ' and subject_id = ' . $request->class_subject_id;
            Log::channel('custom')->error('$sql:' . $sql);
            $teachers = DB::select($sql);

            Log::channel('custom')->error('teachers:' . json_encode($teachers));

            $sql = "select sb.name from class_subjects csb left join subjects sb on csb.subject_id = sb.id where csb.id = " . $request->class_subject_id;
            Log::channel('custom')->error('$sql:' . $sql);
            $subject = DB::select($sql);
            Log::channel('custom')->error('subject:' . json_encode($subject));

            $text = 'online exam ' . $request->title .' datetime: ' . $request->start_date . '~' . $request->end_date . '\n' . $subject[0]->name;
