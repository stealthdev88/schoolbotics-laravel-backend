...
class NotificationController extends Controller
{
    ...
    public function __construct(NotificationInterface $notification, CachingService $cache, UserInterface $user, FeesInterface $fees, SchoolInterface $schoolsRepository1) 
    {
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            Log::channel('custom')->error('notifyUser:' . json_encode($notifyUser));
            $users = DB::select("select * from users where id in (" . implode(',', $notifyUser) . ")");
            ...
            $this->sendSMSForNotificationCreated(Auth::user()->school_id, "notification_created", $users, $message);
