...
class LeaveController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // $text = 'Leave proposal \n' . 'name: ' . Auth::user()->full_name . ' date: ' . $request->from_date . ' ~ ' . $request->to_date . ' ' . count($request->type) . ' days \n' . 'reason: ' . $request->reason . '\n';
            ...
            // $this->sendSMSForLeaveCreated(Auth::user()->school_id, "leave_created_approved", $placeholder, Auth::user());
    ...
    {
        Log::channel('custom')->error('111111');
        ResponseService::noFeatureThenRedirect('Staff Leave Management');
    ...
    public function leave_status_update(Request $request)
    {
        ...
        try {
            ...
            $sql = "select u.*, concat(u.first_name, ' ', u.last_name) full_name from leaves l left join users u on l.user_id = u.id where l.id = " . $request->id;
            $staff_user = DB::select($sql)[0];
            $text = $staff_user->full_name;
            $text1 = $request->status == 0 ? "pending" : ($request->status == 1 ? "approved" : "rejected");
            $text2 = Auth::user()->full_name;

            $settings = app(CachingService::class)->getSystemSettings();
            $placeholder = $this->replacePlaceholdersForSMS(Auth::user(), $settings, $text, $text1, $text2);
            Log::channel('custom')->error('placeholder:' . json_encode($placeholder));
            $this->sendSMSForLeaveCreated(Auth::user()->school_id, "leave_created_approved", $placeholder, $staff_user);
    ...
    private function replacePlaceholdersForSMS($user, $settings, $text = '', $text1 = '', $text2 = '')
    {
