            ...
            <div class="card">
                <div class="card-body">
                    <form class="create-form-without-reset" action="{{ route('system-settings.payment.update') }}" method="POST" novalidate="novalidate" enctype="multipart/form-data">
                        ...
                        <!-- <div class="border border-secondary rounded-lg mb-3" style="display:none;"> -->
                        <div class="border border-secondary rounded-lg mb-3">
                        ...
                        <div class="border border-secondary rounded-lg mb-3">
                            ...
                            <div class="row my-4 mx-1">
                                <div class="form-group col-sm-12 col-md-6">
                                    <label for="paystack_status">{{__("status")}} <span class="text-danger">*</span></label>
                                    <select name="gateway[PayStack][status]" id="paystack_status" class="form-control">
                                        <option value="1" {{(isset($paymentGateway["PayStack"]["status"]) && $paymentGateway["PayStack"]["status"]==1) ? 'selected' : ''}}>{{__("Enable")}}</option>
                                        <option value="0" {{(isset($paymentGateway["PayStack"]["status"]) && $paymentGateway["PayStack"]["status"]==0) ? 'selected' : ''}}>{{__("Disable")}}</option>
                                    </select>
                                </div>
                                <input type="hidden" name="gateway[PayStack][currency_code]" id="paystack_currency" value="{{$paymentGateway["PayStack"]['currency_code'] ?? ''}}">
                                {{-- <div class="form-group col-sm-12 col-md-6">--}}
                                {{-- <label for="paystack_currency">{{__("Currency")}} <span class="text-danger">*</span></label>--}}
                                {{-- <select name="gateway[PayStack][currency_code]" id="paystack_currency" class="form-control select2-dropdown select2-hidden-accessible">--}}
                                {{-- <option value="GHS">GHS</option>--}}
                                {{-- <option value="USD">USD</option>--}}
                                {{-- <option value="AED">AED</option>--}}
                                {{-- <option value="AFN">AFN</option>--}}
                                ...
                                <div class="form-group col-sm-12 col-md-6">
                                    <label for="paystack_publishable_key">{{__("PayStack Publishable Key")}} <span class="text-danger">*</span></label>
                                    <input type="text" name="gateway[PayStack][api_key]" id="paystack_publishable_key" class="form-control" placeholder="PayStack Publishable Key" required value="{{$paymentGateway["PayStack"]['api_key'] ?? ''}}">
                                </div>

                                <div class="form-group col-sm-12 col-md-6">
                                    <label for="paystack_secret_key">{{__("PayStack Secret Key")}} <span class="text-danger">*</span></label>
                                    <input type="text" name="gateway[PayStack][secret_key]" id="paystack_secret_key" class="form-control" placeholder="PayStack Secret Key" required value="{{$paymentGateway["PayStack"]['secret_key'] ??''}}">
                                </div>

                                <div class="form-group col-sm-12 col-md-6">
                                    <label for="paystack_webhook_secret">{{__("PayStack Webhook Secret")}} <span class="text-danger">*</span></label>
                                    <input type="text" name="gateway[PayStack][webhook_secret_key]" id="paystack_webhook_secret" class="form-control" placeholder="PayStack Webhook Secret" required value="{{$paymentGateway["PayStack"]['webhook_secret_key'] ?? ''}}">
                                </div>

                                @if (Auth::user()->school_id)
                                <div class="form-group col-sm-12 col-md-6">
                                    <label for="paystack_webhook_url">PayStack Webhook URL</label>
                                    <input type="text" name="gateway[PayStack][webhook_url]" id="paystack_webhook_url" class="form-control" placeholder="PayStack Webhook URL" disabled value="{{ url('webhook/paystack') }}">
                                </div>
                                @else
                                <div class="form-group col-sm-12 col-md-6">
                                    <label for="paystack_webhook_url">PayStack Webhook URL</label>
                                    <input type="text" name="gateway[PayStack][webhook_url]" id="paystack_webhook_url" class="form-control" placeholder="PayStack Webhook URL" disabled value="{{ url('subscription/webhook/paystack') }}">
                                </div>
                                @endif
                            </div>
...
<script>
    ...
    @if(!empty($paymentGateway["PayStack"]['currency_code'])) {
        {
            $('#paystack_currency').val("{{$paymentGateway['PayStack']['currency_code']}}").trigger("change");
        }
    }
    @endif
